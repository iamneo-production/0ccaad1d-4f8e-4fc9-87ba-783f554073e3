...
<<<<<<< HEAD
  ...
  const fetchOrders = (mail) => {
    axios
      .get(`https://8080-dcbafaeadabbbccbebafcdcbccefeddcbcbaffb.project.examly.io/user/Myorders/${mail}`)
  ...
  const handleDelete = (id) => {
    ...
    setCart(updatedData);
    axios
      .delete(`https://8080-dcbafaeadabbbccbebafcdcbccefeddcbcbaffb.project.examly.io/user/deleteOrder/${id}`)
      .then((response) => {
...
=======
  ...
  const fetchOrders = (mail) =>{
    axios.get(`https://8080-bdedccbceacfdedbebafcdcbccefeddcbcbaffb.project.examly.io/user/Myorders/${mail}`).then((response)=>setCart(response.data.order))
  }
  ...
  const handleDelete = (id) => {
    const updatedData =  cart.filter((item,index) => item.orderId!== id)
    setCart(updatedData)
    axios.delete(`https://8080-bdedccbceacfdedbebafcdcbccefeddcbcbaffb.project.examly.io/user/deleteOrder/${id}`).then(response=>{
      console.log(response)
    })
