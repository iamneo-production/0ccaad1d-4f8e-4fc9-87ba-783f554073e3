...
const Myorder = () => {
  ...
  const fetchOrders = (mail) =>{
    axios.get(`https://8080-bdedccbceacfdedbebafcdcbccfcecaabcfba.project.examly.io/user/Myorders/${mail}`).then((response)=>setCart(response.data.order))
  }
  ...
  const handleDelete = (id) => {
    ...
    setCart(updatedData)
    axios.delete(`https://8080-bdedccbceacfdedbebafcdcbccfcecaabcfba.project.examly.io/user/deleteOrder/${id}`).then(response=>{
