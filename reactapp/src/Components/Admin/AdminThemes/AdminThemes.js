...
const AdminThemes = () => {
  ...
  const themeDelete = (id) => {
    const filteredThemes = themesContainer.filter(
      (theme) => theme.themeId !== id
    );
    setThemes(filteredThemes);
    setThemesContainer(filteredThemes);
    axios
      .delete(`http://localhost:5035/deleteTheme/${id}`, {
        themeId: id,
      })
      .then((response) => {
  ...
  const updateCompleted = (theme, bool) => {
    axios
      .put(`http://localhost:5035/editTheme/${theme.themeId}`, {
      ...
      .then(() => {
        axios.get("http://localhost:5035/admin/getTheme").then((response) => {
  ...
  const performSearch = (search) => {
    const searchThemes = themesContainer.filter((theme) =>
      theme.themeName.toLowerCase().includes(search.toLowerCase())
    );
  ...
  const themeAdd = (name, price, description) => {
    ...
    axios
      .post("http://localhost:5035/admin/addTheme", {
        ThemeName: name,
        ThemePrice: price,
        ThemeDetails: description,
      })
      ...
      <div className="adminTheme-wrapper">
        <div className="theme-wrapper">
          <div className="form-input">
            <input
              ...
              placeholder="Type here to search theme"
              style={serachstyle}
            />
            ...
            themes.map((theme, index) => {
              return (
                <Themes
                  key={theme.themeId}
                  id={theme.themeId}
                  onDelete={themeDelete}
                  themes={theme}
                  editThemeHandler={editTheme}
                />
              );
            })
          )}
        ...
        <div className="addTheme-container">
          {addOrUpdate ? (
            ...
            <EditTheme
              editTheme={editTheme}
              backToHome={updateCompleted}
              updatedValue={beingUpdate}
            />
          )}
        </div>
      </div>
